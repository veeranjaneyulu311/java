...
def probabilty_for_unmatched(unmatched_brand_details, df_for_db_drug_brand_details, df_for_db_drug_brand_details_process):
    ...
    def check_for_matching(brand_details):
        ...
        if (fuzz.ratio(brand_details['drug_brand_nm'], unmatched_brand_name_process) > 65) and (brand_details['drug_brand_nm'] != unmatched_brand_name_process):
            return True
        else:
            return False
    ...
    try:
        ...
        for unmatched_brand_index in df_for_unmatched_brand_details.index:
            ...
            # condition to check given brand name  unmatched with any of brand name in database but at least having approximate matches for suggesting
                ...
                values = ','.join(cursor_two.mogrify("(%s,%s,%s,%s,%s,%s,%s)", (brand['drug_brand_nm'], brand['dose_nm'], brand['drug_type_nm'], unmatched_brand_details['drug_brand_comp_id'].item(), unmatched_brand_details['created_usr_id'].item(), unmatched_brand_details['org_id'].item(), unmatched_brand_details['org_grp_id'].item())).decode('utf-8') for brand in result)
