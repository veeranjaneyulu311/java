...
def compare_excel_uploaded_tests_with_database_tests(request_data, req_url_rule, token):
    try:
        logger.info(
            "{'transactionId':%s,'systemName':%s,'moduleName':'CatalogTestsExcelUpload','timeStamp':%s,'apiName':%s,'status':'START','payLoad':[%s]}",
        ...
        logger.info(
            "{'transactionId':%s,'systemName':%s,'moduleName':'CatalogTestsExcelUpload','timeStamp':%s,'apiName':%s,'status':'END','payLoad':[%s]}",
            token, get_computer_name(), time.strftime('%Y-%m-%d %H:%M:%S'), req_url_rule,
            request_data)
    except Exception:
        conn.rollback()
