...
# unmatched_test_name = unmatched_test_details["long_common_nm"]
    ...
    def approximate(filtered_tests_details):
        if (fuzz.ratio(filtered_tests_details['long_common_nm'], unmatched_test_name_process) > 90) and (
                    filtered_tests_details['long_common_nm'] != unmatched_test_name_process):
            approximately_matched_tests_with_remaining_cols_exactly_match.append(json.loads(df_for_existing_tests.loc[filtered_tests_details.name].to_json()))
    ...
    def check_for_matching(tests_details):
        if (fuzz.ratio(tests_details['long_common_nm'], unmatched_test_name_process) > 65) and (
...
def get_suggestions(conn, request_data, df_for_existing_tests_pre_process, df_for_existing_tests):
    ...
    for unmatched_test_index in df_for_unmatched_test_details_pre_process.index:
        ...
        if has_suggestions:
            values = ','.join(cursor.mogrify("(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
            test['long_common_nm'], test['loinc_system'], test['investigation_department'], test['time_aspect'], test['is_panel'], test['order_obs'],
            unmatched_test_details['lab_test_comp_id'].item(), unmatched_test_details['created_usr_id'].item(),
            unmatched_test_details['org_id'].item(), unmatched_test_details['org_grp_id'].item())).decode('utf-8') for
                              test in result)
            cursor.execute(
                "INSERT INTO lab.lab_test_suggest_ref (long_common_nm,loinc_system,investigation_department,time_aspect,is_panel,order_obs,lab_test_comp_id,created_usr_id,org_id,org_grp_id) VALUES " + values)
        else:
...
def check_for_matches_exist(test_name_in_filtered_data, df_for_uploaded_tests_pre_process, uploaded_test_index):
...
def get_matched_unmatched_data(*args):
    ...
    for uploaded_test_index in df_for_uploaded_tests.index:
        ...
        # print(df_for_existing_tests_pre_process_filtered)
        ...
        if len(df_for_existing_tests_pre_process_filtered.index) != 0:
            df_for_existing_tests_pre_process_filtered['long_common_nm'].apply(func=check_for_matches_exist, args=[df_for_uploaded_tests_pre_process, uploaded_test_index])
...
def fetch_all_brands():
    try:
        # get the payload from given request
        request_data = request.get_json()
        query_to_fetch_all_brands_uploaded = "SELECT long_common_nm, loinc_system, investigation_department, time_aspect, is_panel, order_obs, is_matched from lab.lab_test_comp_ref where org_id = %(org_id)s and process_status = %(status)s"
        conn = threaded_postgreSQL_pool.getconn()
        cursor = conn.cursor(cursor_factory=RealDictCursor)
