...
def data_build_scheduler():
    ...
    # run scheduler daily on total data for employees having data less than one month
    ...
    # run scheduler every month 15th on total data for employees having data more than one month and less than 6 months
    ...
    # run scheduler every 3 months on total data for employees having data more than  6 months and less than one year
...
def fetch_drug_types_and_instructions_for_drug_id():
        ...
        types = []
        for row in rows:
            types.append(dict(zip(('drug_type_id', 'drug_type_nm'), row)))
        instructions = top_results_for_each_user[request_data['authenticatedUserId']]['id-instructions'][request_data['drugId']]
        return jsonify({"data": {"types": types, "instructions": instructions}, "responseCode": "E200", "responseMessage": "DATA FETCHED SUCCESSFULLY"})
...
def fetch_drug_frequencies_for_dose():
    ...
    frequencies = []
    for row in rows:
        frequencies.append(dict(zip(('drug_frequency_id', 'drug_frequency_nm'), row)))
    return jsonify({"data": frequencies, "responseCode": "E200", "responseMessage": "DATA FETCHED SUCCESSFULLY"})
